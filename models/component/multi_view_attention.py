    ...
    def __init__(self, dim=512, way_num=5, shot_num=5, out_type='query', w_type='eye_add'):
        '''
            w_type:
                ...
                point: w参数为单值, 作为权重与原特征相乘, 目前只支持非query
            shot_num:
                目前仅在point模式使用, shot_num = patch_num*origin_shot_num
        '''
        ...
        if self.out_type == 'query':
            ...
            if self.w_type == 'eye_add':
                ...
                self.weight = nn.Parameter(torch.ones(1, way_num, shot_num))  # [1, way_num, shot_num]
            else:  # eye_default
                eye_base = torch.eye(dim)
